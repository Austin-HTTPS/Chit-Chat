...
var secretValues = ['85858dsd4', '5898sds22', '747483sds8', '74489dsd494', '6sds22', '848sd84', '848383', '985748', '484844', '848484', '4949494', '584949', '838hrj', 'jdjdkd', '8383838', 'jdkdkd', '83ienkd', '748829hjd', 'jfjeuhuheuf', 'hefuhehfi', 'jushidjjd', 'ushduhasid4', '6754'];

var numberInalphabet = 15;
...
function resetEventListeners() {
  document.getElementById('user-form').addEventListener('click', e => {
    ...
    var userOfPage = currentUsername;
    ...
    var urlToFetch = "https://mainserver.gamedev46.repl.co/api/add?user=" + userOfPage + "&message=" + message + "&secret=" + secretValues[numberInalphabet];

    if (userOfPage == "GameDev46") {
      urlToFetch = "https://mainserver.gamedev46.repl.co/api/add?user=" + userOfPage + "&message=" + message + "&secret=" + secretValues[numberInalphabet] + "&d=" + secretValues[22];
    }
...
function clearAllMessage() {
  ...
  displayArea.innerHTML = '<form class="text-send" id="text-send" action="#"><input type="text" id="messageFromUser" autocomplete="off" placeholder="What is going on?"><button id="user-form">Send</button></form> <h2 class="refresh" id="refresh"><i class="fas fa-sync-alt"></i></h2>'
...
function decodeAndDisplayDataFromServer(dataFromServer, verifiedOrNot) {
  ...
  for(var z = 0; z < repeatTimesForLoop; z += 2) {
    ...
    userSpeech.innerText = seperatedText[i + 1];
    ...
    reportButton.addEventListener('click', e => {
      ...
      var urlOfReportAdd = "https://mainserver.gamedev46.repl.co/api/reports/add?username=" + usernameOfReportedPerson + "&message=" + messageOfReportedUser;
